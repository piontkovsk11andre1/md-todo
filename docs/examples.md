...
```bash
md-todo run README.md -- opencode run
```

What happens:

1. `md-todo` selects `Rewrite the opening paragraph for the README`
...
This opens the interactive session, lets the user steer it, then returns to `md-todo` for verification and optional repair after exit.
...
```bash
md-todo run . -- opencode run
```

Because the task starts with `cli:`, `md-todo` executes `npm test` directly instead of sending the task to the worker.
...
```bash
md-todo plan README.md -- opencode run
```
...
```bash
md-todo run roadmap.md --vars-file --var ticket=ENG-42 -- opencode run
```

This loads `.md-todo/vars.json`, overrides `ticket`, and makes those values available in templates.
...
This preserves the per-run folder under `.md-todo/runs/` so prompts, logs, and metadata can be inspected later.

## 8. Auto-commit completed tasks

```bash
md-todo run docs/ --commit -- opencode run
```

After a task is verified and checked, `md-todo` stages the Markdown file and commits with a structured message:

```
md-todo: complete "Add installation notes" in docs/setup.md
```

To find when a specific task was completed:

```bash
git log --grep='md-todo: complete' --oneline
```
...
```bash
md-todo run roadmap.md --commit --on-complete "git push" -- opencode run
```

Or use the hook for notifications, logging, or chaining:

```bash
md-todo run roadmap.md --on-complete 'echo "Completed: $MD_TODO_TASK in $MD_TODO_FILE"' -- opencode run
```
