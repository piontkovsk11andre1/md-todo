...
describe("run-task-iteration", () => {
  ...
  it("waits for nearest cooling-down worker and retries resolution once", async () => {
    ...
    const dispatchSpy = vi.spyOn(taskExecutionDispatchModule, "dispatchTaskExecution").mockResolvedValue({
      kind: "ready-for-completion",
      shouldVerify: false,
      cliExecutionOptionsForVerification: undefined,
      verificationFailureMessage: "Verification failed after all repair attempts. Task not checked.",
    ...
    vi.spyOn(completeTaskIterationModule, "completeTaskIteration").mockResolvedValue({
      continueLoop: false,
      exitCode: 0,
    });
    ...
    const iterationPromise = runTaskIteration({
      ...
      worker: {
        workerPattern: inferWorkerPatternFromCommand(["primary", "worker"]),
        loadedWorkerConfig: {
          workers: {
  ...
  it("fails after one cooldown wait when worker remains ineligible", async () => {
    ...
    expect(result).toEqual({ continueLoop: false, exitCode: 1, forceRetryableFailure: false });
    expect(dispatchSpy).not.toHaveBeenCalled();
  ...
  it("emits no-configured-worker error when no worker candidates exist", async () => {
    const cwd = "/workspace";
    const taskFile = path.join(cwd, "tasks.md");
    const task = createTask(taskFile, "Ship release");
    const fileSystem = createInMemoryFileSystem({
  ...
  it("emits health-blocked error when configured workers are unavailable", async () => {
    const cwd = "/workspace";
    const taskFile = path.join(cwd, "tasks.md");
    const task = createTask(taskFile, "Ship release");
    const fileSystem = createInMemoryFileSystem({
...
describe("complete-task-iteration", () => {
  ...
  it("refreshes memory template vars for verify/repair after execute-time memory writes", async () => {
    ...
    const runVerifyRepairLoopSpy = vi
      .spyOn(verifyRepairLoopModule, "runVerifyRepairLoop")
      .mockResolvedValue({
        valid: true,
        failureReason: null,
      });
    ...
    expect(dependencies.memoryResolver.resolve).toHaveBeenCalledWith(memoryTask.file);
    expect(runVerifyRepairLoopSpy).toHaveBeenCalledTimes(1);
    const verifyRepairInput = runVerifyRepairLoopSpy.mock.calls[0]?.[1];
    expect(verifyRepairInput?.templateVars).toMatchObject({
