...
describe("plan-task", () => {
  ...
  it("uses loop planning template in --loop mode", async () => {
    const cwd = "/workspace";
    const markdownFile = path.join(cwd, "roadmap.md");
    const { dependencies, events } = createDependencies({
      cwd,
  ...
  it("falls back to built-in loop template in --loop mode when plan-loop.md is missing", async () => {
    ...
    expect(code).toBe(0);
    const prompt = events.find((event) => event.kind === "text")?.text ?? "";
    expect(prompt).toContain("## Loop composition requirements");
    expect(prompt).toContain("`get:` discovers an iterable set of items/values.");
    expect(prompt).toContain("For loop-oriented tasks, require explicit `get:` + `for:` + `end:` composition");
  });
  ...
  it("leaves legacy prepend/append placeholders unresolved in custom plan templates", async () => {
    const cwd = "/workspace";
    const markdownFile = path.join(cwd, "roadmap.md");
    const { dependencies, events } = createDependencies({
      cwd,
    ...
    expect(code).toBe(0);
    const prompt = events.find((event) => event.kind === "text")?.text ?? "";
    expect(prompt).toContain("prepend={{planPrependGuidance}}");
  ...
  it("does not include prepend/append advisory guidance in deep prompt context", async () => {
    const cwd = "/workspace";
    const markdownFile = path.join(cwd, "roadmap.md");
    const content = "# Roadmap\n- [ ] Build release flow\n";
    const { dependencies, workerExecutor, fileSystem } = createDependencies({
    ...
    expect(deepPrompt).not.toContain("Optional prepend guidance (advisory)");
    expect(deepPrompt).not.toContain("Optional append guidance (advisory)");
