...
function expectedInitSuccessLines(displayConfigDir: string): string[] {
  return [
    ...
    `✔ Created ${displayConfigDir}/agent.md`,
    `✔ Created ${displayConfigDir}/execute.md`,
    `✔ Created ${displayConfigDir}/help.md`,
    `✔ Created ${displayConfigDir}/discuss.md`,
...
describe.sequential("CLI integration", () => {
  ...
  it("init creates .rundown defaults and exits with 0", async () => {
    ...
    expect(fs.existsSync(path.join(workspace, ".rundown", "agent.md"))).toBe(true);
    ...
    expect(fs.existsSync(path.join(workspace, ".rundown", "plan-loop.md"))).toBe(true);
  ...
  it("init --config-dir creates defaults at the explicit target directory", async () => {
    ...
    expect(fs.existsSync(path.join(customConfigDir, "agent.md"))).toBe(true);
    ...
    expect(fs.existsSync(path.join(customConfigDir, "plan-loop.md"))).toBe(true);
