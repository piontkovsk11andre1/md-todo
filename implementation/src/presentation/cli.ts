...
const configCommand = program
  ...
  .addHelpText(
    ...
    [
      ...
      "  - linux:   $XDG_CONFIG_HOME/rundown/config.json, else ~/.config/rundown/config.json",
      "  - macOS:   ~/Library/Application Support/rundown/config.json",
      "             fallback discovery also checks $XDG_CONFIG_HOME and ~/.config",
      "  - windows: %APPDATA%\\rundown\\config.json",
      "             fallback discovery checks %LOCALAPPDATA%, %USERPROFILE%\\AppData\\Roaming, and ~/.config",
      "",
      "Defaults:",
