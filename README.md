...
- `start` scaffolds a prediction project with `design/current/` (default primary doc: `design/current/Target.md`), `AGENTS.md`, `migrations/`, `specs/`, and `.rundown/`.
- `docs publish` snapshots `design/current/` into the next `design/rev.N/` directory, and returns a no-op message when `design/current/` is unchanged from the latest revision.
