...
// Matches ANSI escape/control sequences commonly emitted by terminal output.
const ANSI_ESCAPE_PATTERN = /[\u001B\u009B][[\]()#;?]*(?:(?:(?:[a-zA-Z\d]*(?:;[a-zA-Z\d]*)*)?\u0007)|(?:(?:\d{1,4}(?:;\d{0,4})*)?[\dA-PR-TZcf-ntqry=><~]))/g;

/**
 * Strips ANSI escape sequences from a string value.
 *
 * @param value Input string that may include terminal control codes.
 * @returns Plain-text string with ANSI sequences removed.
 */
